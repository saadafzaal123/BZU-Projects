    ...
    <div id="portfolio" class="container" style = "width:100%; margin-top:-20px; background:#e3e3e3;">
		...
		<div class="module form-module" style = "margin-bottom:50px;">
            ...
            <div class="form">
                ...
                <form action = "{{route('insertShop')}}" method = "post" enctype = "multipart/form-data">
                         ...
                         @foreach($market as $m)
                            {
                                @foreach($city as $c)
                                   @if($m->city_id == $c->id)
                                    <option value = "{{$m->id}}">{{$m->name.' in '.$c->name}} </option>
                                   @endif
                                @endforeach
                            }
                         @endforeach
                    ...
                    @if ($errors->has('shop_img'))
                        <span class="help-block" style = "color: red;">
                        <strong>{{ $errors->first('shop_img') }}</strong>
                    </span>
